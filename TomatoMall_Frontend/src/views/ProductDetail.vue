...
const checkWishListStatus = async () => {
  ...
  try {
    const response = await checkInWishList(product.value.id);
    if (response.code === "200") {
      isInWishList.value = response.data;
    }
  } catch (error) {
...
const handleWishList = async () => {
  ...
  try {
    wishlistLoading.value = true;      const response = await (isInWishList.value ? 
    ...
    if (response.code === "200") {
      ...
      ElMessage.success(isInWishList.value ? "已加入愿望单" : "已从愿望单移除");
    } else {
      throw new Error(response.msg || (isInWishList.value ? "移除失败" : "添加失败"));
    }
  } catch (error) {
    console.error("处理愿望单操作失败:", error);
