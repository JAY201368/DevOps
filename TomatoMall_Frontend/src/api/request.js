...
request.interceptors.request.use(
  config => {
    ...
    // 检查是否是推荐相关的URL、评论相关的URL或促销券相关的URL
    const isRecommendationUrl = config.url.includes('/recommendations');
    const isCommentUrl = config.url.includes('/comments');
    const isCouponUrl = config.url.includes('/coupons');
    
    // 只对GET请求使用缓存，且排除推荐相关的请求、评论相关的请求和促销券相关的请求
    if (config.method.toLowerCase() === 'get' && !isRecommendationUrl && !isCommentUrl && !isCouponUrl) {
...
request.interceptors.response.use(
  response => {
    ...
    const isCouponUrl = response.config.url.includes('/coupons');
    
    // 缓存GET请求的成功响应，但排除推荐、购物车、愿望单、库存、评论和促销券相关的请求
